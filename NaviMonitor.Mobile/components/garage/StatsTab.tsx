  ...
  return (
    <View className="flex-1 px-6 pt-2 pb-32">
      ...
      <View className="flex-row justify-between w-full mb-4">
        <StatCard 
          label="Total Spend"
          value={totalMonthlySpend.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          prefix="₱"
          icon={Wallet}
        />
        <StatCard 
          ...
          <View className="relative items-center justify-center w-[80px] h-[80px]">
            <Svg width="100" height="100" viewBox="0 0 100 100" className="-rotate-90">
              ...
              <Circle 
                ...
                stroke="#b7102a" strokeWidth={strokeWidth} fill="transparent" 
                strokeDasharray={circumference} strokeDashoffset={fuelDashoffset} 
                strokeLinecap="butt"
              />
            </Svg>
            <View className="absolute items-center justify-center">
              <Text className="font-extrabold text-[15px] text-[#1c1b1b] tracking-tighter leading-none">
                {overallGarageSpend > 0 ? `${fuelPercentage}%` : '0%'}
              </Text>
            </View>
          </View>
          ...
          <View className="justify-center gap-2 pr-4">
            <View className="flex-row items-center gap-2">
              ...
              <View>
                ...
                <Text className="font-bold text-xs text-[#1c1b1b] mt-0.5 leading-none">
            ...
            <View className="flex-row items-center gap-2">
              ...
              <View>
                ...
                <Text className="font-bold text-xs text-[#1c1b1b] mt-0.5 leading-none">
                  ₱ {totalServiceCost.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </Text>
              </View>
            </View>
          </View>
      ...
      <View className="bg-[#ffffff] border border-[#e5e2e1] rounded-2xl p-5 shadow-sm flex-1">
        ...
        <View className="flex-1 flex-row items-end justify-between px-4 pt-2 relative">
          ...
          <View className="items-center w-[15%] z-10">
            <View style={{ height: getBarHeightPercent(weeklyTrends[0]) }} className="w-full bg-[#e5e2e1] rounded-t-sm" />
            <Text className="font-bold text-[9px] text-[#848484] mt-2 uppercase">W1</Text>
          </View>
          
          <View className="items-center w-[15%] z-10">
            <View style={{ height: getBarHeightPercent(weeklyTrends[1]) }} className="w-full bg-[#e5e2e1] rounded-t-sm" />
            <Text className="font-bold text-[9px] text-[#848484] mt-2 uppercase">W2</Text>
          </View>
          
          <View className="items-center w-[15%] z-10">
            <View style={{ height: getBarHeightPercent(weeklyTrends[2]) }} className="w-full bg-[#e5e2e1] rounded-t-sm" />
            <Text className="font-bold text-[9px] text-[#848484] mt-2 uppercase">W3</Text>
          </View>
          
          <View className="items-center w-[15%] z-10">
            <View style={{ height: getBarHeightPercent(weeklyTrends[3]) }} className="w-full bg-[#b7102a] rounded-t-sm shadow-sm" />
            <Text className="font-bold text-[9px] text-[#b7102a] mt-2 uppercase">W4</Text>
          </View>
        </View>
      </View>
