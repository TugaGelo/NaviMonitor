...
export default function EditVehicleScreen() {
  ...
  useEffect(() => {
    const loadData = async () => {
      if (id) {
        const data = await VehicleRepository.getVehicleById(Number(id));
        if (data) {
          setForm({
            ...
            engineSizeCC: data.engineSizeCC.toString(),
            startingOdometer: data.startingOdometer.toString(),
  ...
  return (
    <View className="flex-1 justify-end">
      ...
      <Animated.View style={{ height: SCREEN_HEIGHT * 0.665, transform: [{ translateY: slideAnim }, { translateY: keyboardOffset }] }}>
        <View className="flex-1 bg-white rounded-t-[24px] overflow-hidden" style={{ elevation: 24, shadowColor: '#000', shadowOffset: { width: 0, height: -10 }, shadowOpacity: 0.15, shadowRadius: 20 }}>
          ...
          <View className="px-6 pt-5 pb-4 border-b border-[#f3f4f6]">
            <View className="flex-row justify-between items-start">
              <View>
                <Text className="text-2xl font-bold text-[#111827] tracking-tight">Edit Vehicle</Text>
                <Text className="text-sm text-[#6b7280] mt-1">Update your vehicle information</Text>
              </View>
          ...
          <View className="p-5 border-t border-[#f3f4f6] bg-[#f9fafb] pb-8">
            ...
            >
              ...
              <Text className="text-white text-base font-bold ml-1">Update Vehicle</Text>
            </Pressable>
          </View>
      ...
      {showDatePicker && (
        <DateTimePicker
          ...
          onChange={(event, selectedDate) => {
            ...
            if (selectedDate) setForm({ ...form, registrationExpiry: selectedDate.toISOString().split('T')[0] });
          }}
        />
      )}
    </View>
