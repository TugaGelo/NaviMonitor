...
export default function QuickRefuelLogScreen() {
  ...
  const [form, setForm] = useState({ date: new Date().toISOString().split('T')[0], odometer: '', volume: '', totalCost: '', fuelType: '' });
  
  // Settings States
  const [fuelTypes, setFuelTypes] = useState<string[]>(['Unleaded', 'Premium', 'Diesel']);
  ...
  useEffect(() => {
    SettingsRepository.getSettings().then(settings => {
      ...
      if (!editId && settings.fuelTypes.length > 0) {
        setForm(f => ({ ...f, fuelType: settings.fuelTypes[0] }));
      }
    });
  }, []);
  ...
  useEffect(() => {
    if (editId) {
      VehicleRepository.getFuelLogById(Number(editId)).then(log => {
        if (log) setForm({ date: log.date, odometer: log.odometer.toString(), volume: log.volume.toString(), totalCost: log.totalCost.toString(), fuelType: log.fuelType || '' });
      });
    }
  }, [editId]);
    ...
    >
      <View className="flex-row gap-4">
        ...
        <View className="flex-1">
          <StitchInput label="Odometer" required placeholder={currentOdo.toString()} unit={distanceUnit.toLowerCase()} keyboardType="numeric" value={form.odometer} onChange={(v) => setForm({...form, odometer: v})} />
        </View>
      </View>

      <View className="flex-row gap-4">
        <View className="flex-1"><StitchInput label="Volume" required placeholder="0.00" unit={volumeUnit} keyboardType="numeric" value={form.volume} onChange={(v) => setForm({...form, volume: v})} /></View>
